...
class Useropen extends Controller
{
    ...
    public function login($code)
    {
        ...
        return $this->renderLoginSuccess($model, $user_id);
    }
    ...
    public function invite($referee_id = '')
    {
        ...
        $redirectUrl = $oauth->scopes(['snsapi_userinfo'])->redirect($redirect_uri);
    ...
    public function register()
    {
        $data = $this->request->post();
        $model = new UserOpenModel;
        $user_id = $model->phoneRegister($data);
        if ($user_id) {
            return $this->renderLoginSuccess($model, $user_id);
        }
    ...
    /**
     * 登录成功统一返回（与 phonelogin / smslogin 一致）
     */
    private function renderLoginSuccess(UserOpenModel $model, $user_id)
    {
