...
class Product extends ProductModel
{
    ...
    private function setProductListDataFromApi(&$data, $isMultiple, $param)
    {
        return parent::setProductListData($data, $isMultiple, function ($product) use ($param) {
            $userInfo = $param['userInfo'] ?? false;
            $this->setProductGradeMoney($userInfo, $product);
        });
    }
    ...
    {
        $zoneType = (int)($product['zone_type'] ?? 0);
        if (WzNormalZoneProductService::skipsMemberGradeDiscount($zoneType)) {
            $product['is_user_grade'] = false;
            if (WzNormalZoneProductService::isFirstZone($zoneType)) {
                return;
            }
            if (WzNormalZoneProductService::usesTierPricing($zoneType)) {
                WzNormalZoneProductService::applyBuyerPriceToProductDetail($product, $user);
     ...
     * 查询商品所有配送方式
     */
    public function getLogistics($list)
    {
        $allData = [];
        $singleData = [];
        foreach ($list as $product) {
            foreach ($product['logistics'] as $value) {
                $allData[] = $value;
            }
            if (count($product['logistics']) == 1) {
                $singleData[] = $product['logistics'][0];
            }
        }
        //仅支持单个配送方式
        $singleData = array_unique($singleData);
        $singleData = count($singleData) == 1 ? $singleData[0] : '';
        $data = [
            'allData' => $allData,
