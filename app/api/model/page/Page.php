...
class Page extends PageModel
{
    ...
    public static function getPageData($user, $page_id = null)
    {
        ...
        foreach ($items as $key => $item) {
            ...
            if ($item['type'] === 'window') {
                ...
                $items[$key]['data'] = (new ChatRelationModel)->getChatUser(0, $user);
            }
        }
    ...
    public static function getCenterPageData($user, $page_id = 0)
    {
        ...
        foreach ($items as $key => $item) {
            ...
            if ($item['type'] === 'product') {
                $items[$key]['data'] = $model->getProductList($user, $item);
            } elseif ($item['type'] == 'navBar') {
                $items[$key]['data'] = [];
                foreach ($item['data'] as $value) {
                    if ($value['linkUrl'] == '/pages/shop/index' && !$store_open) {
                        continue;
                    } elseif ($value['linkUrl'] == '/pagesPlus/chat/notice_list') {
                        //判断当前用户是否为客服
                ...
                $items[$key]['is_check'] = (new ClerkModel)->isCheck($user);
            }
        }
        return ['page' => $items['page'], 'items' => $items];
    }
    ...
    private function getProductList($user, $item)
    {
        ...
        if ($item['params']['source'] === 'choice') {
            ...
            // 数据朶源：自动
            $autoParams = $item['params']['auto'];
            $params = [
                'type' => 'sell',
                'category_id' => $autoParams['category'] ?? 0,
                'sortType' => $autoParams['productSort'] ?? 'all',
                'list_rows' => $autoParams['showNum'] ?? 6,
                'audit_status' => 10
            ];
            // 添加 zone_type 筛选条件(优品区=1, 消费区=2)
            if (isset($autoParams['zone_type']) && $autoParams['zone_type'] > 0) {
                $params['zone_type'] = (int)$autoParams['zone_type'];
            }
            $productList = $model->getList($params, $user);
        }
        ...
        foreach ($productList as $product) {
            ...
            $data[] = [
                ...
                'green_points' => round((float)($product['green_points'] ?? 0), 2),
                'voucher_max_money' => round((float)($product['voucher_max_money'] ?? 0), 2),
            ];
        }
        return $data;
    }
