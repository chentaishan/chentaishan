...
class UserCoupon extends UserCouponModel
{
     ...
     * 优惠券奖励(根据数量发放)
     */
    public function addNewUserCoupon($coupon_ids, $user_id)
    {
        $model = new CouponModel();
        ...
        foreach ($coupon as $key => $value) {
            ...
            for ($i = 0; $i < $value['coupon_num']; $i++) {
                $data[] = [
                    ...
                    'shop_supplier_id' => $couponInfo['shop_supplier_id'],
                    'user_id' => $user_id,
                    'app_id' => self::$app_id,
                ];
            }
    ...
    public static function getUserCouponList($user_id, $orderPayPrice, $shop_supplier_id)
    {
        ...
        foreach ($list as $coupon) {
            ...
            if ($coupon['coupon_type']['value'] == 20) {
                ...
                $reduced_price = bcsub($orderPayPrice, $reducePrice, 2);
                if ($coupon['max_price'] > 0 && $reduced_price > $coupon['max_price']) {
                    $reduced_price = $coupon['max_price'];
                }
                $data[$key]['reduced_price'] = $reduced_price;
            } else
