...
class Product extends BargainProductModel
{
    ...
    public static function getBargainProduct($params, $user)
    {
        ...
        $order = OrderProductModel::alias('op')
            ...
            ->find();
        if ($order) {
            throw new BaseException(['msg' => '订单已生成']);
        }
        if (empty($task)) {
    ...
    public function getBargainDetail($bargain_product_id)
    {
        ...
        if (!empty($result)) {
            ...
            if ($result['shop_supplier_id']) {
                ...
                $supplier['supplier_user_id'] = (new SupplierUserModel())->where('shop_supplier_id', '=', $result['shop_supplier_id'])->value('supplier_user_id');
                $supplier->visible(['logos', 'category_name', 'name', 'shop_supplier_id', 'product_sales', 'server_score', 'store_type', 'supplier_user_id']);
