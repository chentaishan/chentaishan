...
class CashPaySuccessService extends BaseService
{
    ...
    public function onPaySuccess($payType, $payData = [])
    {
        ...
        if ($status == true) {
            ...
            // 获取订单详情
            $detail = DepositOrderModel::getUserOrderDetail($this->model['order_id'], $this->user['user_id']);
            //小程序发货
            (new DepositOrderModel)->sendWxExpress($detail, $this->user);
        }
        return $status;
    }
    ...
    {
        if ($this->model['shop_supplier_id']) {
            $shop_supplier_id = $this->model['shop_supplier_id'];
        } else {
            $supplier_user = (new SupplierUserModel())->where(['user_id' => $user_id])->where('is_delete', '=', 0)->find();
            $shop_supplier_id = $supplier_user['shop_supplier_id'];
        }
