...
class MasterPaySuccessService extends BaseService
{
    ...
    private function updatePayStatus($payType, $payData = [], $payInfo = [])
    {
        ...
        $this->model->transaction(function () use ($payType, $payData, $payInfo) {
            ...
            // 先扣买家抵扣券、余额（与 order.balance 一致，支付成功后才实扣）
            $this->deductBuyerVoucherBeforeRewards();
            $this->deductBuyerBalanceBeforeRewards();

            // 甲丽华威活动奖励（能量/分区奖）
            $this->bonusToUser($this->model);
    ...
    /** 支付成功：实扣订单预占的抵扣券 */
    private function deductBuyerVoucherBeforeRewards(): void
    {
        try {
            (new GreenPointsService())->deductVoucherOnPaySuccess($this->model->toArray());
        } catch (\Throwable $e) {
            // 不阻断支付成功主流程
        }
    }
    ...
    /**
     * 支付成功：先扣减订单使用的余额，再触发活动奖励（避免代理奖发放时买家余额未扣）
     */
    private function deductBuyerBalanceBeforeRewards(): void
    {
        $deductAmount = round((float)$this->model['balance'], 2);
        if ($deductAmount <= 0) {
            return;
        }

        $orderNo = (string)$this->model['order_no'];
        $exists  = Db::name('user_balance_log')
            ->where('user_id', '=', (int)$this->user['user_id'])
            ->where('scene', '=', BalanceLogSceneEnum::CONSUME)
            ->where('describe', 'like', '%' . $orderNo . '%')
            ->value('log_id');
        if ($exists) {
            return;
        }
    ...
    private function bonusToUser($order): void
    {
        try {
            $orderData = is_array($order) ? $order : (method_exists($order, 'toArray') ? $order->toArray() : (array)$order);
            if (empty($orderData['zone_type']) && !empty($orderData['order_id'])) {
                $orderData['zone_type'] = (int)Db::name('order')
                    ->where('order_id', '=', (int)$orderData['order_id'])
                    ->value('zone_type');
            }
            (new ActivityRewardService())->onPaySuccess($orderData);
        } catch (\Throwable $e) {
            // 奖励异常不阻断支付成功
        }
    }
