...
abstract class OrderSettledService extends BaseService
{
    ...
    public function settlement()
    {
        ...
        foreach ($this->supplierData as &$supplier) {
            $zone_type = isset($supplier['productList'][0]['zone_type']) ? $supplier['productList'][0]['zone_type'] : 9;
            foreach ($supplier['productList'] as &$zoneProduct) {
                WzNormalZoneProductService::applyBuyerPriceToProduct($zoneProduct, $this->user);
            }
            unset($zoneProduct);
            // 整理订单数据
        ...
        // 计算积分商城抵扣
        $this->setOrderPoints($orderPayPrice, $this->settledRule['force_points']);
        // 计算完成后再计算积分可抵扣
        $delivery = [];
        foreach ($this->supplierData as &$supplier) {
            $this->orderData = $supplier['orderData'];
            // 商品总价 - 积分抵扣（抵扣券在支付页 use_voucher 使用）
        ...
        //计算佣金
        $this->setOrderCommissionMoney();
        $this->commonOrderData['is_use_points'] && $orderPayPrice = $orderPayPrice - $this->commonOrderData['points_money'];
        $this->commonOrderData['zone_type'] = $product['zone_type'] ?? 0;
        // 计算订单积分赠送数量
        $this->setOrderPointsBonus();
        // 计算订单绿色积分赠送
        $this->setOrderGreenPointsBonus();
        // 计算订单可用抵扣券上限（仅消费区；Σ voucher_max_money×数量，未配置则不可用）
        $this->setOrderVoucherMaxMoney();
        $delivery = array_unique($delivery);
        // 是否显示收货地址
        if (in_array(DeliveryTypeEnum::EXPRESS, $delivery)) {
            $this->commonOrderData['show_address'] = 1;
        }
        // 是否显示自提信息
        if (in_array(DeliveryTypeEnum::EXTRACT, $delivery)) {
            $this->commonOrderData['show_extract'] = 1;
        }
        //订单数据
    ...
    private function getCommonOrderData()
    {
        ...
        $exchangeConfig = GreenPointsService::supportsSchema()
            ? GreenPointsService::getExchangeConfig()
            : [];
        return [
            // 默认地址
            ...
            'green_points_exchange' => $exchangeConfig,
            // 支付方式
    ...
    public function createOrder($order)
    {
        ...
        foreach ($order['supplierList'] as $supplier) {
            if (!$supplier['productList']) {
                continue;
            }
            if (count($supplier['orderData']['logistics']) == 1 && $supplier['orderData']['delivery'] != $supplier['orderData']['logistics'][0]) {
                $this->error = '当前商品配送方式选择错误';
                return false;
            } elseif (count($supplier['orderData']['logistics']) == 2 && $supplier['orderData']['is_support'] == 1) {
                $this->error = $supplier['supplier']['name'] . '部分商品不支持当前配送方式，请重新下单';
                return false;
            }
    ...
    private function setOrderProductPayPrice($productList)
    {
        ...
        foreach ($productList as &$product) {
            ...
            if (isset($product['coupon_money']) && $value > 0) {
                $value = helper::bcsub($value, $product['coupon_money']);
            }
            // 减去积分抵扣金额
            if ($this->orderData['is_allow_points'] && $this->commonOrderData['is_use_points'] && !$this->settledRule['force_points'] && $value > 0) {
                $value = helper::bcsub($value, $product['points_money'] ? $product['points_money'] : 0);
            }
            // 减去商品满减金额
            if ($product['product_reduce_money'] > 0 && $value > 0) {
                $value = helper::bcsub($value, $product['product_reduce_money']);
            }
            // 减去满减金额
            if ($this->settledRule['is_reduce'] && $this->orderData['reduce'] && $value > 0) {
                $value = helper::bcsub($value, $product['fullreduce_money'] ? $product['fullreduce_money'] : 0);
            }
            $product['total_pay_price'] = helper::number2($value);
        }
        return true;
    ...
    private function getOrderData($shop_supplier_id, $productList)
    {
        ...
        if ($productList[0]['is_virtual'] == 1) {
            ...
            $logisticsData = (new ProductModel)->getLogistics($productList);
            $logistics = array_unique($logisticsData['allData']);
            ...
            } elseif (count($logistics) == 2 && $logisticsData['singleData']) {
                $delivery = $logisticsData['singleData'];
                $deliveryType = [$logisticsData['singleData']];
            }
            ...
            //查询不支持的商品
            foreach ($productList as $product) {
                if (!in_array($delivery, $product['logistics'])) {
                    $is_support = 1;
                    break;
                }
            }
    ...
    private function validateOrderForm(&$order)
    {
        ...
        if ($order['orderData']['show_address'] == 1 && !$order['orderData']['address']) {
            $this->error = '请添加地址';
            return false;
        }
        if ($order['orderData']['show_extract'] == 1) {
            if (empty($this->params['linkman']) || empty($this->params['phone'])) {
                $this->error = '请填写收货人姓名和收货人电话';
                return false;
            }
        }
        return true;
    ...
    private function createOrderEvent($commomOrder, $supplier)
    {
        ...
        // 积分兑换扣除用户积分
        if ($commomOrder['force_points'] && $supplier['orderData']['points_num'] > 0) {
    ...
    private function add($commomOrder, $supplier)
    {
        ...
        $data = [
            ...
            'green_points_bonus' => $supplier['orderData']['green_points_bonus'] ?? 0,
            'voucher_money' => 0,
            ...
            'zone_type'=>$commomOrder['zone_type'],
        ];
        if ($data['room_id'] > 0) {
            $data['order_source'] = OrderSourceEnum::LIVE;
        }
        if ($supplier['orderData']['delivery'] == DeliveryTypeEnum::EXPRESS) {
    ...
    private function saveOrderProduct($supplier, $status, $commomOrder)
    {
        ...
        foreach ($supplier['productList'] as $product) {
            $item = [
                ...
                'green_points_bonus' => isset($product['green_points_bonus']) ? $product['green_points_bonus'] : 0,
                'voucher_money' => 0,
        ...
        if (isset($supplier['buyProduct']) && $supplier['buyProduct']) {
            $this->OrderBuyProduct($supplier['buyProduct'], $productList, $status);
        }
     ...
     */
    private function setOrderPoints($orderTotalPrice, $force_points = true)
    {
        $productList = [];
        foreach ($this->supplierData as &$supplier) {
            foreach ($supplier['productList'] as $product) {
            ...
            $supplier['orderData']['points_money'] = 0;
            $supplier['orderData']['points_num'] = 0;
            $supplier['orderData']['is_allow_points'] = false;
        }
        // 设置默认数据：订单信息
        helper::setDataAttribute($this->commonOrderData, [
            'points_money' => 0,       // 积分抵扣总金额
            'points_num' => 0,    // 积分抵扣总数量
        ], false);
        // 积分商城兑换
        if (isset($this->settledRule['force_points']) && $this->settledRule['force_points'] && $force_points) {
            // 积分抵扣金额，商品价格-兑换金额
            $this->supplierData[0]['orderData']['points_money'] = $productList[0]['points_money'];
            // 积分抵扣总数量
            $this->supplierData[0]['orderData']['points_num'] = $productList[0]['points_num'];
            // 允许积分抵扣
            $this->supplierData[0]['orderData']['is_allow_points'] = true;
        ...
        // 条件：订单金额满足[?]元
        if (helper::bccomp($setting['discount']['full_order_price'], $orderTotalPrice) === 1) {
            return false;
        }
        // 计算订单商品最多可抵扣的积分数量
        ...
        if ($actualPointsNum < 1) {
            foreach ($this->supplierData as &$supplier) {
                $supplier['orderData']['points_money'] = 0;
                $supplier['orderData']['points_num'] = 0;
                $supplier['orderData']['is_allow_points'] = true;
            }
            return false;
        }
        // 计算订单商品实际抵扣的积分数量和金额
        ...
        // 统计供应商的分配额度
        foreach ($this->supplierData as &$supplier) {
            $supplier['orderData']['points_money'] = helper::number2(helper::getArrayColumnSum($supplier['productList'], 'points_money'));
            $supplier['orderData']['points_num'] = helper::number2(helper::getArrayColumnSum($supplier['productList'], 'points_num'));
            $supplier['orderData']['is_allow_points'] = true;
        }
        // 积分抵扣总金额
        $orderPointsMoney = helper::getArrayColumnSum($productList, 'points_money');
        $this->commonOrderData['points_money'] = helper::number2($orderPointsMoney);
        // 积分抵扣总数量
        $this->commonOrderData['points_num'] = $actualPointsNum;
        // 允许积分抵扣
        $this->commonOrderData['is_allow_points'] = true;
        return true;
    }
     ...
     * 计算订单积分赠送数量
     * 支持两种积分赠送方式:
     * 1. 后台按比例赠送: is_points_gift=1, 按支付金额*比例计算
     * 2. 商品固定赠送: gift_points > 0, 直接使用该固定值(2026-04-15新增)
     */
    private function setOrderPointsBonus()
    {
        ...
        foreach ($this->supplierData as &$supplier) {
            foreach ($supplier['productList'] as &$product) {
                // 优先使用商品的 gift_points 固定值(如果设置了)
                $giftPoints = (int)($product['gift_points'] ?? 0);
                if ($giftPoints > 0) {
                    // 使用商品固定赠送积分
                    $product['points_bonus'] = $giftPoints;
                } else if ($product['is_points_gift']) {
                    // 使用后台按比例赠送
                    $ratio = $setting['gift_ratio'] / 100;
                    $product['points_bonus'] = helper::bcmul($product['total_pay_price'], $ratio, 0);
                } else {
                    // 不赠送积分
                    $product['points_bonus'] = 0;
                }
    ...
    private function setOrderGrade($productList)
    {
        ...
        foreach ($productList as &$product) {
            ...
            if ($product['is_alone_grade'] && isset($product['alone_grade_equity'][$this->user['grade_id']])) {
                ...
                $discountRatio = $equity > 0 ? helper::bcdiv($equity, 100) : 1;
            }
            if ($discountRatio < 1) {
                // 会员折扣后的商品总金额
    ...
    private function setOrderSysCouponMoney($couponId, $couponList)
    {
        ...
        // 分配订单商品优惠券抵扣金额
        foreach ($productList as $key => &$product) {
            if (isset($completed[$key]['coupon_money_sys']) && $completed[$key]['coupon_money_sys'] > 0) {
                $product['coupon_money_sys'] = $completed[$key]['coupon_money_sys'] / 100;
            } else {
                $product['coupon_money_sys'] = 0;
            }
        }
        // 统计供应商的分配额度
        foreach ($this->supplierData as &$supplier) {
    ...
    /**
     * 计算订单绿色积分赠送（消费区：商品 green_points × 数量）
     */
    private function setOrderGreenPointsBonus()
    {
        foreach ($this->supplierData as &$supplier) {
            foreach ($supplier['productList'] as &$product) {
                $product['green_points_bonus'] = 0;
            }
            $supplier['orderData']['green_points_bonus'] = 0;
        }
        if ((int)($this->commonOrderData['zone_type'] ?? 0) !== WzNormalZoneProductService::ZONE_NORMAL) {
            return false;
        }
        ...
        foreach ($this->supplierData as &$supplier) {
            foreach ($supplier['productList'] as &$product) {
                $per = (float)($product['green_points'] ?? 0);
                if ($per > 0) {
                    $product['green_points_bonus'] = (float)helper::bcmul((string)$per, (string)(int)$product['total_num'], 2);
                }
            }
            $supplier['orderData']['green_points_bonus'] = helper::getArrayColumnSum($supplier['productList'], 'green_points_bonus');
        }
        return true;
    }

    /**
     * 结算台：可用抵扣券上限（支付页 use_voucher 预校验）
     */
    private function setOrderVoucherMaxMoney()
    {
        $zoneType = (int)($this->commonOrderData['zone_type'] ?? 0);
        foreach ($this->supplierData as &$supplier) {
            $payPrice = round((float)($supplier['orderData']['order_pay_price'] ?? 0), 2);
            $maxMoney = GreenPointsService::calcCheckoutVoucherMax($zoneType, $supplier['productList'], $payPrice);
            $supplier['orderData']['max_voucher_money'] = $maxMoney;
            $supplier['orderData']['can_use_voucher'] = $maxMoney > 0 ? 1 : 0;
        }
        $totalPay = round((float)($this->commonOrderData['order_pay_price'] ?? 0), 2);
        $allProducts = [];
        foreach ($this->supplierData as $supplier) {
            $allProducts = array_merge($allProducts, $supplier['productList']);
        }
        $this->commonOrderData['max_voucher_money'] = GreenPointsService::calcCheckoutVoucherMax($zoneType, $allProducts, $totalPay);
        $this->commonOrderData['can_use_voucher'] = ($this->commonOrderData['max_voucher_money'] ?? 0) > 0 ? 1 : 0;
        return true;
    }
     ...
     * 获取所有支付价格
     */
    private function setOrderCommissionMoney()
    {
        foreach ($this->supplierData as &$supplier) {
            //查询平台抽成
            $sys_percent = (new SupplierModel)->where('shop_supplier_id', '=', $supplier['shop_supplier_id'])->value('commission_rate');
            // 平台分佣金额
            $supplier['orderData']['sys_money'] = helper::number2(($supplier['orderData']['order_price'] + $supplier['orderData']['coupon_money']) * $sys_percent / 100);
            if ($this->commonOrderData['is_use_points']) {
                $points_money = $supplier['orderData']['points_money'];
            } else {
                $points_money = 0;
            }
            // 供应商结算金额，包括运费
            $supplier['orderData']['supplier_money'] = helper::number2($supplier['orderData']['order_price'] + $supplier['orderData']['coupon_money_sys'] + $points_money - $supplier['orderData']['sys_money'] + $supplier['orderData']['express_price']);
            // 结算金额不包括运费
            foreach ($supplier['productList'] as &$product) {
                if ($this->commonOrderData['is_use_points']) {
                    $product_points_money = $product['points_money'];
                } else {
                    $product_points_money = 0;
                }
                $product['sys_money'] = helper::number2(($product['total_pay_price'] + $product['coupon_money']) * $sys_percent / 100);
                $product['supplier_money'] = helper::number2($product['total_pay_price'] + $product['coupon_money_sys'] + $product_points_money - $product['sys_money']);
            }
        }
        return true;
    }
