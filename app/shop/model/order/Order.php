...
class Order extends OrderModel
{
    ...
    public function getReviewOrderTotal()
    {
        $filter['pay_status'] = OrderPayStatusEnum::SUCCESS;
        $filter['order_status'] = 10;
        return $this->where($filter)->where('delivery_status', '<>', 20)->count();
    }
     ...
     * 优品区商品总金额
     */
    public function getFirstZoneMoney()
    {
        return $this->where('pay_status', '=', 20)
            ->where('order_status', '<>', 20)
            ->where('is_delete', '=', 0)
     ...
     * 消费区总金额
     */
    public function getRebuyZoneMoney()
    {
        return $this->where('pay_status', '=', 20)
            ->where('order_status', '<>', 20)
            ->where('is_delete', '=', 0)
     ...
     * 消费区总积分
     */
    public function getRebuyZonePoints()
    {
        return Db::name('activity_reward_log')
            ->where('status', '=', 1)
            ->where('zone_type', '=', 2)
            ->sum('amount');
    }
     ...
     * 区域代理提成（优品区省/市/区）
     */
    public function getStoreCommission($zoneType = 1)
    {
        return Db::name('activity_reward_log')
            ->where('status', '=', 1)
            ->where('zone_type', '=', $zoneType)
            ->whereIn('scene', ['region_province', 'region_city', 'region_district'])
            ->sum('amount');
    }
     ...
     * 消费区拓客补贴
     */
    public function getDirectPushReward($zoneType = 2)
    {
        return Db::name('activity_reward_log')
            ->where('status', '=', 1)
            ->where('zone_type', '=', $zoneType)
            ->whereIn('scene', ['direct_push'])
            ->sum('amount');
    }
