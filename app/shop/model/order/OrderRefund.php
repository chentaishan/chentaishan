...
class OrderRefund extends OrderRefundModel
{
    ...
    public function getList($query = [])
    {
        ...
        // 商户
        if (isset($query['shop_supplier_id']) && $query['shop_supplier_id'] > 0) {
            $model = $model->where('m.shop_supplier_id', '=', $query['shop_supplier_id']);
        }

        // 获取列表数据
        $list = $model->alias('m')
            ->field('m.*, order.order_no')
            ->with(['orderproduct.image', 'orderMaster.advance', 'user', 'supplier'])
