...
use app\common\service\activity\WzProductZonePriceValidateService;
...
class Product extends ProductModel
{
    ...
    public function add(array $data)
    {
        ...
        if ($msg = WzProductZonePriceValidateService::validate($data)) {
            $this->error = $msg;
            return false;
        }
    ...
    public function edit($data)
    {
        ...
        //查询当前商户配送方式
        $logistics = (new SupplierModel())->getLogistics($this['shop_supplier_id']);
        if (count($logistics) == 1) {
            $data['logistics'] = [$logistics[0]['value']];
        }
    ...
    {
        if (ProductService::checkProductLocked($this)) {
     ...
     * 获取数量
     */
    public function getCount($type, $params)
    {
        $model = $this->applyEnabledSupplierScope($this->alias('product'));
        ...
        if (isset($params['product_name']) && !empty($params['product_name'])) {
            $model = $model->where('product.product_name', 'like', '%' . trim($params['product_name']) . '%');
        }
        ...
        // 销售中
        if ($type == 'sell') {
            $model = $model->where('product.audit_status', '=', 10);
            $model = $model->where('product.product_status', '=', 10);
        }
        //仓库中
        if ($type == 'lower') {
            $model = $model->where('product.audit_status', '=', 10);
            $model = $model->where('product.product_status', '=', 20);
        }
        // 回收站
        if ($type == 'recovery') {
            $model = $model->where('product.product_status', '=', 30);
        }
        //待审核
        if ($type == 'audit') {
            $model = $model->where('product.audit_status', '=', 0);
        }
        //未通过
        if ($type == 'no_audit') {
            $model = $model->where('product.audit_status', '=', 20);
        }
