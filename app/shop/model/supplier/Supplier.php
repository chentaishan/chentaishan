...
class Supplier extends SupplierModel
{
    ...
    public function add($data)
    {
        ...
        try {
            ...
            $supplier = $this->normalizeSupplierPayload($supplier);
    ...
    public function setDelete()
    {
        ...
        try {
            //删除关联此用户的申请记录
            (new Apply())->where('user_id', '=', $this['user_id'])->update(['is_delete' => 1]);
            //删除关联此商户的申请记录
            (new Apply())->where('shop_supplier_id', '=', $this['shop_supplier_id'])->update(['is_delete' => 1]);
            //删除商户登录账号
            (new SupplierUserModel)->where('shop_supplier_id', '=', $this['shop_supplier_id'])->update(['is_delete' => 1]);
            //修改会员
    ...
    public static function getSupplierTotalByDay($day = "")
    {
        ...
        if ($day) {
            ...
            $model = $model->where('create_time', 'between', [$startTime, $startTime + 86399]);
        }
     ...
     * 获取平台的总销售额
     */
    public function getTotalMoney($type = 'total')
    {
        $model = $this;
        if ($type == 'total') {
     ...
     * 统一处理商户新增/编辑入参
     */
    private function normalizeSupplierPayload(array $supplier): array
    {
        if (!array_key_exists('is_show', $supplier) || $supplier['is_show'] === '') {
            $supplier['is_show'] = 1;
        } else {
            $supplier['is_show'] = (int)$supplier['is_show'] > 0 ? 1 : 0;
        }
        if (array_key_exists('latitude', $supplier) && $supplier['latitude'] !== '') {
            $supplier['latitude'] = (string)$supplier['latitude'];
        }
        if (array_key_exists('longitude', $supplier) && $supplier['longitude'] !== '') {
            $supplier['longitude'] = (string)$supplier['longitude'];
        }
        return $supplier;
    }
}