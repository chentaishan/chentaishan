...
class Page extends PageModel
{
    ...
    public function setDelete()
    {
        ...
        try {
            ...
            //首页装修无数据新增
            if ($this['page_type'] == 10 || $this['page_type'] == 30) {
                $pageCount = $this->where('is_delete', 0)->where('page_type', $this['page_type'])->count();
                if ($pageCount <= 0) {
                    self::addDefault($this['page_type'], $this['app_id']);
                }
            }
            $this->commit();
            return true;
            ...
            if ($item['type'] === 'product' && $item['params']['source'] === 'choice') {
                foreach ($item['data'] as $key => $value) {
                    ...
                    if ($productInfo) {
                        ...
                        $item['data'][$key]['line_price'] = $productInfo['line_price'];
