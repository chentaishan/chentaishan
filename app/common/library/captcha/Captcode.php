...
class Captcode
{
     ...
     * 验证验证码是否正确
     * @access public
     * @param string $code 用户输入验证码,$uniqid: key
     * @return array 用户验证码是否正确,200为成功
     */
    public function check(string $code, string $uniqid)
    {
        $cacheCode = Cache::get($uniqid);
        if (is_null($cacheCode) || !$cacheCode) {
            return ["code" => 0, "msg" => '验证码失效了'];
        }
