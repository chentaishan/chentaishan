...
class Product extends BaseModel
{
    ...
    protected $append = ['product_sales', 'delivery_name'];

    /**
     * 不支持配送方式
     */
    public function getDeliveryNameAttr($value, $data)
    {
        $name = "";
        if (isset($data['logistics']) && $data['logistics']) {
            $logistics = explode(',', $data['logistics']);
            if (count($logistics) == 1) {
                $name = $logistics[0] == 10 ? '不支持自提' : '不支持快递';
            }
        }
        return $name;
    }
     ...
     * @return array
     */
    public function setLogisticsAttr($value)
    {
     ...
     * @return array
     */
    public function getLogisticsAttr($value)
    {
    ...
    {
        return $this->hasOne('app\\common\\model\\file\\UploadFile', 'file_id', 'content_video_id');
    }

    /**
     * 关联详情视频封面
     */
    public function contentPoster()
    {
        return $this->hasOne('app\\common\\model\\file\\UploadFile', 'file_id', 'content_poster_id');
    }
    ...
    public function getList($param)
    {
        ...
        if (isset($params['zone_type']) && $params['zone_type'] > 0) {
            $model = $model->where('zone_type', '=', $params['zone_type']);
        }
        // 排序规则
        $sort = [];
        ...
        $list = $this->applyEnabledSupplierScope($model->alias('product'))
    ...
    /**
     * 仅统计/展示正常营业商户的商品（与禁用商户 is_recycle=1 等保持一致）
     */
    protected function applyEnabledSupplierScope($model, string $productAlias = 'product')
    {
        return $model
            ->join('supplier supplier', "{$productAlias}.shop_supplier_id = supplier.shop_supplier_id", 'left')
    ...
    protected function setProductListData($data, $isMultiple = true, callable $callback = null)
    {
        ...
        foreach ($dataSource as &$product) {
            ...
            // 回调函数（如甲丽华威分区价、会员折扣会改写 sku 单价）
            is_callable($callback) && call_user_func($callback, $product);
            // 同步默认展示规格，避免 product_sku 仍为改价前的快照（详情页常用此字段）
