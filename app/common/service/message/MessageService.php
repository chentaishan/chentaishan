...
class MessageService
{
    ...
    public function payment($order, $orderType = OrderTypeEnum::MASTER)
    {
        ...
        //发送公众号模板消息
        if ($settings['mt_status'] == 1 && $order['user']['mpopen_id'] != '') {
            MtMessageService::send($data, $settings['mt_template'], $order['user']['mpopen_id'], $order['app_id']);
        }
        //发送公众号订阅消息
    ...
    public function refund($refund, $order_no, $sence = 'audit')
    {
        ...
        //发送公众号模板消息
        if ($settings['mt_status'] == 1 && $refund['user']['mpopen_id'] != '') {
            MtMessageService::send($data, $settings['mt_template'], $refund['user']['mpopen_id'], $refund['app_id']);
        }
        //发送公众号订阅消息
    ...
    public function agent($agent)
    {
        ...
        //发送公众号模板消息
        if ($settings['mt_status'] == 1 && $user['mpopen_id'] != '') {
            MtMessageService::send($data, $settings['mt_template'], $user['mpopen_id'], $user['app_id']);
        }
        //发送公众号订阅消息
    ...
    public function supplier($order, $orderType = OrderTypeEnum::MASTER)
    {
        ...
        //发送公众号模板消息
        if ($settings['mt_status'] == 1 && $user && $user['mpopen_id'] != '') {
            MtMessageService::send($data, $settings['mt_template'], $user['mpopen_id'], $user['app_id']);
        }
        //发送短信消息
