...
class Product extends ProductModel
{
    ...
    public function add(array $data)
    {
        ...
        try {
            //查询当前商户配送方式
            $logistics = (new SupplierModel())->getLogistics($data['shop_supplier_id']);
    ...
    public function edit($data, $shop_supplier_id)
    {
        ...
        //查询当前商户配送方式
        $logistics = (new SupplierModel())->getLogistics($shop_supplier_id);
        if (count($logistics) == 1) {
            $data['logistics'] = [$logistics[0]['value']];
        }
    ...
    private function addProductSpec($data, $isUpdate, $productSkuIdList = [])
    {
        ...
        if ($data['spec_type'] == '10') {
            ...
            // 单规格：兼容前端未传 line_price
            if (!isset($data['sku']['line_price'])) {
                $data['sku']['line_price'] = 0;
            }
            ...
            // 多规格：兼容前端未传 line_price
            foreach ($data['spec_many']['spec_list'] as &$specItem) {
                if (!isset($specItem['spec_form']['line_price'])) {
                    $specItem['spec_form']['line_price'] = 0;
                }
            }
            unset($specItem);
            // 添加商品与规格关系记录
     ...
     * 根据时间获取当前商品总数
     */
    public function getProductDateTotal($shop_supplier_id, $day = "")
    {
        $model = $this;
        if ($day) {
